...
Выгружает задачи из одного проекта Jira (пространства)
С УЧЁТОМ:
  - фильтра по дате создания (created)
  - фильтра по типу задачи (issuetype = "Release 2.0")

Фильтр:
    project = KEY
    AND issuetype = "Release 2.0"
...
# Тип задачи, по которому фильтруем
ISSUE_TYPE_NAME = "Release 2.0"                # нужный тип (issuetype.name)
...
# --- Выходной файл ---
OUTPUT_FILE = f"jira_{JIRA_PROJECT_KEY}_Release2_0_export_all_fields_with_date.json"
...
def build_jql_for_project_with_dates_and_type(
    project_key: str,
    issue_type_name: str,
    ...
    """
    Собирает JQL с ограничениями по:
      - проекту
      - типу задачи (issuetype)
      - дате created

    Примеры:
      project = "SHIP" AND issuetype = "Release 2.0"
      project = "SHIP" AND issuetype = "Release 2.0" AND created >= "2024-01-01"
      project = "SHIP" AND issuetype = "Release 2.0" AND created <= "2024-12-31"
      project = "SHIP" AND issuetype = "Release 2.0" AND created >= "2024-01-01" AND created <= "2024-12-31"
    """
    parts = [
        f'project = "{project_key}"',
        f'issuetype = "{issue_type_name}"',
    ]
...
def fetch_all_issues_for_project(
    ...
    """
    Забирает задачи из проекта Jira со всеми полями,
    с ограничением по типу задачи и дате создания (created).
    """
    jql = build_jql_for_project_with_dates_and_type(
        project_key,
        issue_type_name,
        created_from,
        created_to,
...
def main():
    logging.info("Старт jira_export_all_fields_with_date.py (Release 2.0)")
