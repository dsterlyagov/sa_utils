...
НАСТРОЙКИ ЧЕРЕЗ .env / ENV:
  ...
  CONF_USER=user.login           # может быть пустым при PAT
  ...
  # Если не указано, берётся директория скрипта.
  BASE_DIR=C:/Users/.../jira-tasks-mfd

  # Пути относительно BASE_DIR:
  META_SCRIPT=widget-store_3/scripts/build-meta-from-zod.ts
  META_OUTDIR=widget-store_3/output
  META_OUTFILE=widget-meta.json

  # Путь к published-widgets.json (относительный к BASE_DIR или абсолютный)
  PUBLISHED_JSON=widget-store_3/published-widgets.json
...
def _load_dotenv(path: Path) -> None:
    ...
    try:
        ...
        print(f"[ERROR] Не удалось загрузить .env: {e}", file=sys.stderr)
...
def _maybe_run_ts(script: Path, project_root: Path, timeout: int) -> None:
    ...
    Если ни один не сработал — логируем и продолжаем работу со старым JSON.
    """
    print(f"[INFO] Попытка запустить TS-скрипт: {script} (root={project_root})")
    for cmd in _pick_runners(script, project_root):
        try:
            _run_ts_once(cmd, project_root, timeout_sec=timeout)
            print("[INFO] TS-скрипт успешно отработал.")
            return
        except Exception as e:
            print(f"[WARN] Runner failed: {' '.join(cmd)}")
            print(f"       Reason: {e}")
    print("[WARN] Не удалось запустить TS-скрипт ни одним раннером — используем существующий JSON.")
...
def _load_published_versions(path: Optional[Path]) -> Dict[str, Dict[str, Optional[int]]]:
    ...
    if not path:
        return {}
    if not path.exists():
        print(f"[WARN] published-json не найден: {path}")
        return {}
    ...
    try:
        ...
        print(f"[WARN] Не удалось прочитать published-json {path}: {e}")
        return {}

    if not isinstance(data, list):
        print(f"[WARN] published-json {path} должен быть массивом")
        return {}
...
def _wait_for_file(path: Path, wait_sec: int) -> None:
    ...
    print(f"[INFO] Ожидание файла: {path}")
    deadline = time.time() + wait_sec
    while time.time() < deadline:
        if path.exists() and path.stat().st_size > 0:
            print(f"[INFO] Найден файл: {path} ({path.stat().st_size} bytes)")
            return
        time.sleep(0.2)
...
def main() -> None:
    ...
    # 0. Подтягиваем значения из .env ИМЕННО из директории скрипта
    ...
    print(f"[INFO] Confluence URL={conf_url}, pageId={page_id}")
    ...
    print(f"[INFO] META_SCRIPT={script}")
    ...
    print(f"[INFO] META_OUTDIR={outdir}")
    ...
    print(f"[INFO] META_OUTFILE={outfile}")
    ...
    print(f"[INFO] Загружено виджетов из meta: {len(widgets)}")

    # 5. Готовим published-widgets (последние версии по DEV/IFT)
    published_versions = _load_published_versions(published_path)
    print(f"[INFO] Загружено записей из published-widgets: {len(published_versions)}")
    ...
    print(f"[INFO] Всего виджетов для рендера: {len(items_for_render)}")
    ...
    print("[INFO] Обновляем страницу Confluence...")
    ...
    print(f"[OK] ✅ Обновлено: pageId={page_id} (версия {next_version})")
